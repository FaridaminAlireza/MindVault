...
* det(A) ≠ 0 → space is scaled but not collapsed 
→ matrix is invertible.
* det(A) = 0 → space is flattened to a lower dimension 
...
* Apply A to a unit cube: 
edges e1, e2, e3 transform to A*e1, A*e2, A*e3
* Volume of transformed parallelepiped = |det(A)|
* If det(A) = 0 → cube collapses → 
transformation flattens space → no inverse.
...
vectors lie in a plane → no inverse.
...
* If det(A) = 0 → Volume_new = 0 → 
  space collapses → no inverse


7. Summary                                                                             |

- det(A) ≠ 0: 
 Matrix is invertible, transformation preserves volume                                   |
- det(A) = 0:
Matrix is singular, 
columns/rows dependent, 
transformation collapses space → no inverse 

"""


"""
The determinant of a matrix (A) tells you:
how much A scales volume (or area) when it transforms space

When you apply (A):

* A single point → moves somewhere else
* A shape (square/cube/region) → gets transformed
* that shape’s volume/area is scaled by: det(A)

What actually gets scaled

If you take:
* a square (2D area = 1)
* or a cube (3D volume = 1)

and apply A, its area/volume becomes:
|A| × original volume
...
So:
* a unit square → becomes a parallelogram with area 6
* a unit cube (if extended in 3D analog) 
→ volume scales similarly


Important insight:

If you take many vectors forming a shape:
x₁, x₂, x₃ forming a cube/parallelepiped

Then:
volume(Ax₁, Ax₂, Ax₃) = |det(A)| × volume(x₁, x₂, x₃)
