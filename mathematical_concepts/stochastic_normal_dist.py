...
2. Fubini/Tonelli theorem:
   * Tonelli: 
   if a function is nonnegative 
   on a product domain,then the 
   double integral equals 
   the iterated integrals
   and we may interchange 
   sums/limits/integrals.

   * Fubini: 
   if the double integral of 
   the absolute value is finite 
   (absolute integrability), then 
   iterated integrals equal the double integral
   and you may change the order of integration.

   * We will invoke Tonelli/Fubini 
   to justify turning a product of integrals
   into a double integral and
...
Assume B is a complex number with 
Re(B) > 0, so that e^{−B x^2} decays 
for large |x| and 
the integral converges absolutely.
For most applied uses 
B is a positive real number; 
from now on consider B > 0 (real) for simplicity — 
I will note where the complex-case difference appears.

---
 STEP-BY-STEP DERIVATION 
# (classical method: square + polar)
...
We introduced a second, 
independent dummy variable y 
for the second factor. 
This is legitimate because 
each integral is just a number 
and dummy variable names do not matter.

Step 2 — Write the product as a double integral.
Because x and y are independent and
the integrand is nonnegative when B > 0,
Tonelli’s theorem applies and we may write:
...
Then x^2 + y^2 = r^2 
and the area element dx dy becomes r dr dθ. 
The entire plane corresponds to 
r ∈ [0, ∞), 
θ ∈ [0, 2π). Thus:
...
Since I = ∫_{−∞}^{∞} e^{−B x^2} dx is a positive real number
whenever B > 0, we take the positive square root:
...
If B is complex with Re(B) > 0, 
the same identity holds but
the square root is taken using
the principal square root branch
consistent with analytic continuation 
(the derivation requires slightly more
care about contours, but the Tonelli/Fubini
step uses absolute convergence because 
Re(B) > 0).

---
 JUSTIFICATIONS

1. Squaring is algebraically valid: 
I^2 is the product of the two identical real numbers.

2. Replacing the second integrand variable x by y is
legitimate because the integration variable is a dummy symbol.

3. Tonelli/Fubini justification: 
for B > 0, the function e^{−B(x^2 + y^2)} 
is nonnegative and integrable over R^2;
Tonelli guarantees that the double integral equals
the iterated integrals and that we may form the 
double integral from the product of single integrals.

4. Polar change-of-variables is valid with 
Jacobian r because the mapping (r, θ) → (x, y) 
is smooth almost everywhere and covers the plane 
with r ≥ 0, θ ∈ [0, 2π). Standard change-of-variables
theorem for integrals supplies dx dy = r dr dθ.

5. u-substitution in the r-integral is routine 
and justified since the substitution is
monotone and differentiable on [0, ∞).

6. Taking the positive square root is valid
because each single integral is positive 
(integrand positive for all x when B > 0).
---
 ALTERNATIVE VIEW (product-of-1D integrals approach)

Instead of starting from I and squaring,
one can note directly:

∫*{−∞}^{∞} e^{−B x^2} dx * ∫*{−∞}^{∞} e^{−B y^2} dy
= ∫∫_{R^2} e^{−B(x^2+y^2)} dx dy

and then proceed with the polar calculation.
This is the same as the previous approach but
highlights that the double integral is separable
into the product of one-dimensional integrals.
...
1. Standard Gaussian normalization (example):
   ...
   φ(x) = (1/√(2π σ^2)) e^{−(x−μ)^2/(2σ^2)}, 
   the normalization constant comes from 
   the Gaussian integral. 
   For B = 1/(2σ^2) and centered at μ = 0,
   we need

   ∫_{−∞}^{∞} e^{−x^2/(2σ^2)} dx = sqrt(2π) σ.

Proof: set B = 1/(2σ^2) in the main result:
 sqrt(π / B) = sqrt(π * 2σ^2) = sqrt(2π) σ.

2. Multi-dimensional Gaussian (n dimensions):
   Let x ∈ R^n and
   consider ∫_{R^n} e^{−B ||x||^2} d^n x. 
   Because the integral factors in 
   Cartesian coordinates or
   by using spherical coordinates, one gets:

∫_{R^n} e^{−B ||x||^2} d^n x = (π / B)^{n/2}.

For example, 
for n = 2 this gives π / B, 
consistent with I^2 above. 
For n = 3, the integral equals (π/B)^{3/2}, etc.

3. Non-isotropic quadratic forms:
   If the exponent is −x^T A x with A positive-definite 
   symmetric matrix, then ∫_{R^n} e^{−x^T A x} d^n x = 
   (π)^{n/2} / sqrt(det(A)). This follows by
   diagonalizing A and changing variables.
...
Justification: square the integral, write the square as
a double integral, convert to polar coordinates 
(dx dy = r dr dθ), evaluate θ-integral to get 2π, 
then evaluate radial integral by substitution u = B r^2 to
obtain I^2 = π / B, and take positive square root.
"""
