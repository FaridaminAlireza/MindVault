...
1) The Root-Finding Problem
 ...
 f(x) = x^n - a = 0
 We can find x by solving this equation 
 iteratively using a root-finding method.


2) Newton-Raphson Method

 Newton's method improves an approximate root x_k 
...
This is the formula used in most calculators and math libraries.
 ...
 x_{k+1} = (x_k + a / x_k) / 2
 This specific case is also known as 
 the Babylonian method for square roots.


3) Why Newton’s Method Works Well

 - It converges quadratically near the root, 
 meaning the number of correct digits roughly 
 doubles at each iteration.
 - Usually, only a few iterations (3 to 6) 
 are needed for double precision.
 - It’s purely arithmetic, so it can run efficiently
on binary floating-point hardware.

4) Choosing a Good Initial Guess

 Newton’s method converges fast only if the initial guess (x_0)
...
We can extract (m, e) using 
the function `frexp(a)` in C or Python.
...
So, we can approximate:
 ...
 For the 2^(e/n) part, we can split e/n 
...
def nth_root_newton(a: float, n: int,
                    tol: float = 1e-15,
    ...
    # ldexp(x, k) = x * (2 ** k)
    init_guess = (math.ldexp(1.0, int_part) *
                   (2.0 ** frac_part) *
                   (m ** (1.0 / n)))
