...
or changes direction at a specific point. 

A line has zero curvature because it does not bend,
while a small circle has a large curvature because 
it changes direction quickly, 
and a large circle has a small curvature because 
it changes direction slowly. 

Mathematically, curvature can be defined as the magnitude
...
3. Evaluate at x = 0: 0.5

4. Evaluate at x = 1: 5.590169943749475
...
* T = unit tangent vector along the curve
* s = arc length along the curve
* Intuition: curvature measures how fast
the tangent vector changes as you move along the curve

**Example:** Circle of radius R

* Position vector r(s) = (R cos(s/R), R sin(s/R))
* Unit tangent T(s) = (-sin(s/R), cos(s/R))
* Derivative of T w.r.t s = (-cos(s/R)/R, -sin(s/R)/R)
* Norm = 1/R → curvature = 1 / R
...
* Derived from intrinsic definition using chain rule
* Intuition: numerator measures “area” formed by velocity
and acceleration, which reflects bending
...
* x' = -R sin(t), x'' = -R cos(t)
* y' = R cos(t), y'' = -R sin(t)
* Numerator = R^2, denominator = R^3 → curvature = 1 / R

---
# 3. Function y = f(x) (parametric with x = t)

Curvature kappa(x) = | f''(x) | / ( 1 + (f'(x))^2 )^(3/2)

* Special case of parametric plane curve
* Intuition: combines slope and concavity to measure bending

**Example:** f(x) = x^2

* f'(x) = 2x, f''(x) = 2
* Curvature kappa(x) = 2 / (1 + (2x)^2)^(3/2)
* At x = 0 → curvature = 2 (maximum at vertex)

---

# 4. Space curve in 3D, r(t)

Curvature kappa(t) = || r'(t) cross r''(t) || / || r'(t) ||^3

* Intuition: cross product gives the perpendicular 
component of acceleration → measures bending in 3D
* Denominator normalizes by speed cubed

**Example:** Helix r(t) = (a cos t, a sin t, b t)

* r'(t) = (-a sin t, a cos t, b)
* r''(t) = (-a cos t, -a sin t, 0)
* Cross product r' × r'' = (b a sin t, -b a cos t, a^2)
* Norm = a * sqrt(a^2 + b^2), speed cubed = (a^2 + b^2)^(3/2)
* Curvature = a / (a^2 + b^2) → constant along the helix
"""