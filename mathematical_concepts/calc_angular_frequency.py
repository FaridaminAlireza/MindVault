...
Symbol: f Unit: Hertz (Hz) = cycles per second 
Meaning:
How many complete cycles happen per second.
Example: If a signal repeats 10 times per second: f = 10 Hz

2) Angular (Circular) Frequency (ω)

Symbol: ω Unit: radians per second (rad/s) 
Meaning: 
How fast the phase angle changes in radians per second.
...
Then: 5 cycles happen in 1 second 
...
This happens when:
- Using sine/cosine 
- Using complex exponentials e^(iθ) 
...
Constant rotation speed → constant angular frequency.
Sine wave = projection of rotation.

So: 
f = how many laps per second 
ω = how fast the angle is sweeping

Mathematically: ω = dθ/dt
